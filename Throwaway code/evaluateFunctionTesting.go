...
// TODO: ASK NATHAN WHY consts aren't in guts.go in pest
...
func evaluate(courseName string, assignmentName string, userName string) error {
	...
	if err != nil {
		fmt.Println("DB error (Assignments)")
		return errors.New("DB error")
	}
	...
	if rows.Next() == false {
		...
		rows.Scan(&results.numTestCases, &results.maxRuntime, "-Wall -std=c++0x") //&results.compilerOptions)
		rows.Close()
	}

	results.submissionNum = 2 //, err = getLastSubmissionNum(courseName, assignmentName, results.userID)
	...
	assignmentRoot := "data/" + results.courseName + "/" + "Assignment " + results.assignmentName + "/"

	// make sure assignmentRoot folder exists, if not, panic
	_, err = os.Stat(assignmentRoot)

	if err != nil {
		fmt.Println("Assignment folder does not exist or permission error: " + assignmentRoot)
...
func main() {
	...
	//evaluate("TerwilligerCS15501SP17", "0", "jdoes")
	//evaluate("TerwilligerCS15501SP17", "66", "jdoe")
	evaluate("TerwilligerCS15501SP17", "0", "jdoe")
}
